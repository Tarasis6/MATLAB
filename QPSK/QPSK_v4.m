...
IQrCoarseFreqCompensate = IQr(1:Fs/Br:end).^ 4;
IQrfCoarseFreqCompensate = IQrf(1:Fs/Br:end).^4;

% figure
%  for k=1:length(IQrfCoarseFreqCompensate)
%      plot(real(IQrCoarseFreqCompensate(1:k)),imag(IQrCoarseFreqCompensate(1:k)),'.');
%      axis([-6,6,-6,6]);
%      pause(0.2)
%  end
plot(real(IQrCoarseFreqCompensate),imag(IQrCoarseFreqCompensate),'.');
axis([-5,5,-5,5]);

Scfo = fftshift(fft(IQrCoarseFreqCompensate));
figure
plot(abs(Scfo));

ff = [ -length(IQrCoarseFreqCompensate)/2:length(IQrCoarseFreqCompensate)/2 - 1]/length(IQrCoarseFreqCompensate) .* Br;
[v,c] = max(abs(Scfo));
dfo = (c - length(IQrCoarseFreqCompensate)/2) .* Br/length(IQrCoarseFreqCompensate)/4;